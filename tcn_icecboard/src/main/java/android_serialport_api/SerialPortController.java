...
public class SerialPortController {
     ...
     * 得到串口
     */
    public SerialPort getSerialPortMDB(String devicekey, String baudratekey)
            throws SecurityException, IOException, InvalidParameterException {
        if (null == m_SerialPortMDB) {
            /* Read serial port parameters */
            SharedPreferences sp = mContext.getSharedPreferences(mContext.getPackageName() + "_preferences", Context.MODE_PRIVATE);
            String path = sp.getString(devicekey, "");
            int baudrate = Integer.decode(sp.getString(baudratekey, "9600"));
			/* Check parameters */
            if ((path.length() == 0) || (baudrate == -1)) {
                throw new InvalidParameterException();
            }
            /* Open the serial port */
            m_SerialPortMDB = new SerialPort(new File(path), baudrate, 0);
            // mSerialPort = new SerialPort(new File("/dev/ttyS0"), baudrate,0);
        }

        return m_SerialPortMDB;
    }
     ...
     * 得到串口
     */
    public SerialPort getSerialPortMDB(String dev,int baudrate) throws SecurityException, IOException, InvalidParameterException {
        if (m_SerialPortMDB == null) {
            /* Open the serial port */
            m_SerialPortMDB = new SerialPort(new File(dev), baudrate,0);
        }
...
//            TcnVendIF.getInstance().LoggerDebug(TAG, "2 InvalidParameterException e: " + e);
     ...
     * 打开并监视串口
     */
    public void openSerialPortMDB(String devicekey, String baudratekey) {

        try {
            //m_SerialPort = m_application.getSerialPort("/dev/ttyS1", 19200);//（写死串口）
            m_SerialPortMDB = getSerialPortMDB(devicekey, baudratekey);

            m_OutputStreamMDB = m_SerialPortMDB.getOutputStream();
            m_InputStreamMDB = m_SerialPortMDB.getInputStream();

            if (null != m_ReadThreadMDB) {
                m_ReadThreadMDB.interrupt();
                m_ReadThreadMDB = null;
            }
            m_ReadThreadMDB = new ReadThreadMDB();
            m_ReadThreadMDB.setName("ReadThreadMDB");
            m_ReadThreadMDB.setRun(true);
            m_ReadThreadMDB.setPriority(Thread.MAX_PRIORITY);
            m_ReadThreadMDB.start();
...
//            TcnVendIF.getInstance().LoggerDebug(TAG, "3 InvalidParameterException e: " + e);
     ...
     * 打开并监视串口
     */
    public void openSerialPortMDB(String dev,int baudrate) {
        try {
            //m_SerialPort = getSerialPort("/dev/ttyS1", 19200);//（写死串口）
            m_SerialPortMDB = getSerialPortMDB(dev, baudrate);
            m_OutputStreamMDB = m_SerialPortMDB.getOutputStream();
            m_InputStreamMDB = m_SerialPortMDB.getInputStream();
    ...
    public void closeSerialPortMDB() {
        if (m_OutputStreamMDB != null) {
            try {
                m_OutputStreamMDB.close();
            } catch (IOException e) {
                e.printStackTrace();
            }
            m_OutputStreamMDB = null;
        }

        if (m_InputStreamMDB != null) {
            try {
                m_InputStreamMDB.close();
            } catch (IOException e) {
                e.printStackTrace();
            }
            m_InputStreamMDB = null;
        }
        if (m_SerialPortMDB != null) {
            m_SerialPortMDB.close();
            m_SerialPortMDB.closeStream();
            m_SerialPortMDB = null;
        }
    }
...
//            TcnVendIF.getInstance().LoggerError(TAG, "writeDataImmediatelyFourth IOException e：" + e);
    ...
    public void writeDataImmediatelyMDB(byte[] bytes) {
        if ((null == m_OutputStreamMDB) || (null == bytes)) {
            return;
        }
        try {
            m_OutputStreamMDB.write(bytes);
            m_OutputStreamMDB.flush();
        } catch (IOException e) {
            e.printStackTrace();
//            TcnVendIF.getInstance().LoggerError(TAG, "writeDataImmediatelyMDB IOException e：" + e);
        }
    }
...
//                    TcnVendIF.getInstance().LoggerError(TAG, "m_InputStreamFourth IOException e: " + e);
    ...
    private class ReadThreadMDB extends Thread {
        ...
        public void run() {
            ...
            if (null == m_InputStreamMDB) {
//                TcnVendIF.getInstance().LoggerError(TAG, "ReadThreadMDB m_InputStreamMDB is null");
                return;
            }
//            TcnVendIF.getInstance().LoggerDebug(TAG, "进入ReadThreadMDB啦------getRun(): " + getRun());
            byte[] buffer = null;
            // String readNew = "";
            int byteCount = 0;
            ...
            while (bIsRun && !isInterrupted()) {
                ...
                try {
                    buffer = new byte[512];
                    byteCount = m_InputStreamMDB.read(buffer);
                    if (byteCount > 0) {
                        if (m_ReceiveHandlerMDB != null) {
                            Message message = m_ReceiveHandlerMDB.obtainMessage();
                            message.what = SERIAL_PORT_RECEIVE_DATA_MDB;
                            message.arg1 = byteCount;
                            message.obj = buffer;
                            m_ReceiveHandlerMDB.sendMessage(message);
                        }
                    }
                } catch (IOException e) {
//                    TcnVendIF.getInstance().LoggerError(TAG, "m_InputStreamMDB IOException e: " + e);
                    e.printStackTrace();
                    break;
                }
            }
