...
public class WriteThread  extends Thread {
    ...
    public void setBusy(boolean busy) {
        if (!m_bNotShowLog) {
//            TcnBoardIF.getInstance().LoggerDebug(TAG, "setBusy() busy: "+busy);
        }
        m_bIsBusy = busy;
        if (!busy) {
            m_bWriting = false;
    ...
    private void addMsgToSendList(MsgToWrite msg)
    {
        ...
        synchronized (m_sendMsgList) {
            ...
            if (!m_bNotShowLog) {
//             TcnBoardIF.getInstance().LoggerDebug(TAG, "tcnxxx.............addMsgToSendList data " + TcnUtility.bytesToHexString(msg.getData())+" m_bIsBusy: "+m_bIsBusy+" getTime: "+msg.getCmdTime());
            }
        ...
        {
            if (!m_bNotShowLog) {
//                TcnBoardIF.getInstance().LoggerDebug(TAG, "addMsgToSendList notify ");
            }
            m_bHasNewDada = true;
            notify();
        }
    ...
    private boolean writeData(int serialPortType, byte[] data,boolean notShowLog)
    {
        ...
        try
        {
            ...
            } else if (SERIAL_PORT_TYPE_MDB == serialPortType) {
                SerialPortController.getInstance().writeDataImmediatelyMDB(data);
            }
            else {
    ...
    private boolean writeData(int serialPortType, String data,boolean notShowLog)
    {
        ...
        try
        {
            ...
            if (SERIAL_PORT_TYPE_1 == serialPortType) {
                ...
                SerialPortController.getInstance().writeDataImmediatelyMDB(data.getBytes());
            }
            else {
    ...
    public void sendMsg(int serialPortType, int cmdType, long cmdOverTimeSpan, byte[] data, boolean notShowLog)
    {
        if (data != null) {
            if (!m_bNotShowLog) {
//               TcnBoardIF.getInstance().LoggerDebug(TAG, "sendMsg data " + TcnUtility.bytesToHexString(data)+" m_bIsBusy: "+m_bIsBusy);
            }
            if (null == m_sendMsg) {
    ...
    public void sendMsg(int serialPortType, int cmdType, int slotNo, long cmdOverTimeSpan, byte[] data, boolean notShowLog)
    {
        if (data != null) {
            if (!m_bNotShowLog) {
//               TcnBoardIF.getInstance().LoggerDebug(TAG, "sendMsg data " + TcnUtility.bytesToHexString(data)+" m_bIsBusy: "+m_bIsBusy);
            ...
            if (null == m_sendMsg) {
                m_sendMsg = new MsgToWrite(serialPortType,cmdType,slotNo, cmdOverTimeSpan, data,notShowLog);
            } else {
                m_sendMsg.cleanData();
                m_sendMsg.setMsgToWrite(serialPortType,cmdType,slotNo, cmdOverTimeSpan, data,notShowLog);
            }
            addMsgToSendList(m_sendMsg);
        }
    }
        ...
        while (m_bIsRun) {
            ...
            synchronized (m_sendMsgList) {
                ...
                while (m_sendMsgList.size() > 0) {
                    ...
                    if (!m_bNotShowLog) {
//                        TcnBoardIF.getInstance().LoggerDebug(TAG, "while m_sendMsgList size: "+msgToWriteList.size());
                    }

                    for (MsgToWrite msg : msgToWriteList) {
                        if (!m_bNotShowLog) {
//                            TcnBoardIF.getInstance().LoggerDebug(TAG, "tcnxxx.run getData " + TcnUtility.bytesToHexString(msg.getData())+" getCmdTime: "+msg.getCmdTime()+" m_bIsBusy: "+m_bIsBusy+" m_bReSend: "+m_bReSend+" m_bWriting: "+m_bWriting);
                        }
                        if (!m_bWriting) {
                            ...
                            while (m_bReSend) {
                                ...
                                while (m_bWriting) {
                                    ...
                                    if (isCmdReciveOverTime(lTime,m_curretnMsg.getOverTimeSpan())) {
                                        if (!m_bNotShowLog) {
//                                            TcnBoardIF.getInstance().LoggerDebug(TAG, "while m_bReSend m_iReSendCount: "+m_iReSendCount+" m_bIsBusy: "+m_bIsBusy);
                                        }
                                        ...
                                        if (!m_bNotShowLog) {
//                                            TcnBoardIF.getInstance().LoggerDebug(TAG, "while m_bReSend end m_iReSendCount: "+m_iReSendCount+" m_bIsBusy: "+m_bIsBusy);
                                        }
                                        if (m_bWriting) {
                                            m_bWriting = false;
                                            m_bIsBusy = false;
                                ...
                                if (!m_bNotShowLog) {
//                                     TcnBoardIF.getInstance().LoggerDebug(TAG, "tcnxxx..m_bReSend " + m_bReSend+" m_bIsBusy: "+m_bIsBusy+" getData: "+TcnUtility.bytesToHexString(m_curretnMsg.getData())+" getCmdTime: "+m_curretnMsg.getCmdTime());
                                }
                            ...
                            if (!m_bNotShowLog) {
//                                TcnBoardIF.getInstance().LoggerDebug(TAG, "while data " + TcnUtility.bytesToHexString(msg.getData())+" m_bIsBusy: "+m_bIsBusy);
                            }
                        ...
                        if (!m_bNotShowLog) {
//                            TcnBoardIF.getInstance().LoggerDebug(TAG, "end m_bIsBusy: "+m_bIsBusy+" m_bWriting: "+m_bWriting);
                        }
                        ...
                        while (m_bWriting) {
                            sleepTime(20);
                            if (!m_bNotShowLog) {
//                                TcnBoardIF.getInstance().LoggerDebug(TAG, "lTime: "+lTime+" msg.getOverTimeSpan(): "+msg.getOverTimeSpan());
                            }

                            if (isCmdReciveOverTime(lTime,msg.getOverTimeSpan())) {
                                if (!m_bNotShowLog) {
//                                    TcnBoardIF.getInstance().LoggerDebug(TAG, "while m_iReSendCount: "+m_iReSendCount+" m_bIsBusy: "+m_bIsBusy);
                                }
                                ...
                                if (!m_bNotShowLog) {
//                                    TcnBoardIF.getInstance().LoggerDebug(TAG, "while end m_iReSendCount: "+m_iReSendCount+" m_bIsBusy: "+m_bIsBusy);
                                }
                            ...
                            if (!m_bNotShowLog) {
//                                TcnBoardIF.getInstance().LoggerDebug(TAG, "while sleep m_bIsBusy: "+m_bIsBusy+" m_bWriting: "+m_bWriting);
                            }

                        }
                        if (!m_bNotShowLog) {
//                            TcnBoardIF.getInstance().LoggerDebug(TAG, "for end size: "+m_sendMsgList.size()+" msgToWriteList size: "+msgToWriteList.size());
                        }

                    }
                    if (!m_bNotShowLog) {
//                        TcnBoardIF.getInstance().LoggerDebug(TAG, "while end m_bIsBusy: "+m_bIsBusy);
                    }
            ...
            if (!m_bNotShowLog) {
//                TcnBoardIF.getInstance().LoggerDebug(TAG, "while end to wait m_bIsBusy: "+m_bIsBusy+" 11 m_bHasNewDada: "+m_bHasNewDada);
            }
            ...
            synchronized (this)
            {
                try
                {
                    if (!m_bHasNewDada) {
                        if (!m_bNotShowLog) {
//                            TcnBoardIF.getInstance().LoggerDebug(TAG, "wait ");
                        }
                        wait();
                    }
                } catch (InterruptedException e)
