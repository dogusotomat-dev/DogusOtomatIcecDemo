...
import static com.tcn.icecboard.def.TcnProtoResultDef.BOARD_LIFT_HEFAN_ZP;
...
public class BoardGroupControl {
    ...
    public void initialize(String board1,String board2,String board3,String board4,String group1,String group2,String group3,String group4) {
        ...
        if ((TcnConstant.DEVICE_CONTROL_TYPE[1]).equals(board1)) {   //冰淇淋
            ...
            for (int i = 0; i < mGroupList.size(); i++) {
                ...
                mGroupInfo.setBoardType(BOARD_ICE);
                mGroupInfo.setMaxSlotNo(getMaxSlotNo(BOARD_ICE,mGroupList.get(i),0));
        ...
        if ((TcnConstant.DEVICE_CONTROL_TYPE[1]).equals(board2)) {
            ...
            for (int i = 0; i < mGroupList.size(); i++) {
                ...
                mGroupInfo.setBoardGrpNo(mGroupList.get(i));
                mGroupInfo.setBoardType(BOARD_ICE);
                mGroupInfo.setMaxSlotNo(getMaxSlotNo(BOARD_ICE,mGroupList.get(i),iMaxSlotNo));
                m_GroupInfoList.add(mGroupInfo);
            }
        }
        ...
        if ((TcnConstant.DEVICE_CONTROL_TYPE[1]).equals(board3)) {
        ...
        iMaxId = getMaxId();
        iMaxSlotNo = getMaxSlotNo();
        if ((TcnConstant.DEVICE_CONTROL_TYPE[1]).equals(board4)) {
    ...
    public List<GroupInfo> getGroupListHefanZp() {
        List<GroupInfo> m_list = new ArrayList<GroupInfo>();
        for (GroupInfo info:m_GroupInfoList) {
            if (info.getBoardType() == BOARD_LIFT_HEFAN_ZP) {
                m_list.add(info);
            }
        }
        return m_list;
    ...
    public int getAddrSlotNo(int boardType,int slotNo) {
        int iAddrSlotNo = slotNo;
        if (slotNo < 100) {
            return iAddrSlotNo;
        }

        if (BOARD_LIFT_HEFAN_ZP == boardType) {
            iAddrSlotNo = slotNo % 300;
        } else {
            iAddrSlotNo = slotNo % 100;
        }
    ...
    public GroupInfo getGroupInfoNextHave(int serGrp, byte boardGrp) {
        int index = -1;
        GroupInfo mGroupInfo = null;
        for (GroupInfo info:m_GroupInfoList) {
            if ((info.getSerGrpNo() == serGrp) && (info.getBoardGrpNo() == getGroupNum(boardGrp))) {
    ...
    public boolean hasIceMachine() {
        boolean bRet = hasMachine(BOARD_ICE);
        return bRet;
    }
    ...
    public boolean hasLiftHefanZpMachine() {
        boolean bRet = hasMachine(BOARD_LIFT_HEFAN_ZP);
        return bRet;
    }
